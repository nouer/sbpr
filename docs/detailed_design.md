...
* **systolic**: 最高血圧（整数, 50〜300）。`noMedication === true` のときは null 可。
* **diastolic**: 最低血圧（整数, 30〜200）。`noMedication === true` のときは null 可。
* **noMedication**: 服薬しなかった日フラグ（boolean, オプション）。`true` のときは血圧未測定として扱い、systolic/diastolic は null。
...
### 2.2 バリデーション仕様
* **noMedication === true** のとき: systolic/diastolic は不要。measuredAt は必須。
* **noMedication 未設定または false** のとき（通常の血圧記録）:
...
### 3.1 血圧分類
JSH2019家庭血圧基準に基づく血圧分類:
* 正常血圧: 収縮期 < 115 かつ 拡張期 < 75
* 正常高値血圧: 収縮期 115-124 かつ 拡張期 < 75
* 高値血圧: 収縮期 125-134 または 拡張期 75-84
* I度高血圧: 収縮期 135-144 または 拡張期 85-89
* II度高血圧: 収縮期 145-159 または 拡張期 90-99
* III度高血圧: 収縮期 >= 160 または 拡張期 >= 100
...
### 3.3 統計情報の表示
* 記録数は「○件 / ○日間」形式で表示される
* 日数は `measuredAt` のローカル日付をユニーク集計して算出する（同日に複数回記録した場合も1日としてカウント）
* `updateStats(records)` 内で `Set` を用いて重複排除し、`stat-count-unit` 要素に動的に反映する
...
| aiModel | string | 任意 | 利用AIモデルID（後方互換のため任意） |
| chartSettings | object | 任意 | グラフ設定（後方互換のため任意） |
| chartSettings.dayStartHour | number | 任意 | 日中の開始時刻（0〜23） |
| chartSettings.nightStartHour | number | 任意 | 夜間の開始時刻（0〜23） |

#### 後方互換性
* `profile`、`aiMemo`、`aiModel`、`chartSettings` はオプショナルフィールド
* 旧バージョンのエクスポートファイル（これらのフィールドなし）もインポート可能
* 旧バージョンのアプリでインポートしてもオプショナルフィールドは無視されるだけで動作に影響なし
...
### 6.2 ツールチップの閉じ動作
* グラフのデータポイントをタップするとツールチップが表示される
* 同じデータポイントを再度タップするとツールチップが閉じる
* 異なるデータポイントをタップすると、前のツールチップが消えて新しいツールチップが表示される
* グラフの空白部分をタップするとツールチップが閉じる
* グラフ外（他のUI要素）をタップしてもツールチップが閉じる
* **実装**:
  * Chart.js の `onClick` ハンドラで前回タップした要素を `lastTooltipIndex` に記録し、同一要素の再タップ時または空白タップ時に `chart.tooltip.setActiveElements([])` でツールチップを非表示にする
  * `initChartTooltipDismiss()` で `document` レベルの `click` リスナーを登録し、キャンバス外クリック時にツールチップを閉じる
* 連続モード・日中/夜間モード・日中のみモード・夜間のみモードの全モードで動作
* PC（マウス操作）・モバイル（タッチ操作）の両方で一貫した動作

### 6.3 グラフ表示モード
* `currentChartMode` の取りうる値: `'continuous'`（連続）, `'daynight'`（日中・夜間）, `'day'`（日中のみ）, `'night'`（夜間のみ）
* 「日中のみ」: `isDaytime()` が true のレコードのみを連続モード形式で描画
* 「夜間のみ」: `isDaytime()` が false のレコードのみを連続モード形式で描画
* **ボタン色の差別化**: モード選択ボタンはインディゴ（#6366f1）、期間選択ボタンはティール（#0d9488）で、行の役割を視覚的に区別する

### 6.4 日中/夜間の線種切り替えトグル
* 日中/夜間モード時のみ表示されるトグルスイッチ
* デフォルト: 日中=実線、夜間=破線
* トグルON時: 日中=破線、夜間=実線（線種が入れ替わる）
* **UI**: `#chart-linestyle-toggle` 内のチェックボックス型トグル
* **表示制御**: `currentChartMode === 'daynight'` のときのみ表示
* **状態管理**: `lineStyleSwapped` (boolean) で管理
* トグル変更時に `refreshChart()` を呼び出してグラフを再描画
* 脈拍の線種も連動して入れ替わる

### 6.5 日中/夜間モードの破線の薄色表示
* 日中/夜間モードで、破線（非メイン）側のデータセットの `borderColor` と `pointBackgroundColor` にアルファ値 0.35 を適用
* メインの実線側は従来通り不透明な色を使用
* `lineStyleSwapped` の状態に応じて、どちら側を薄くするかが切り替わる
...
### 8.2 診断範囲の期間選択
* AI診断タブに期間選択ボタンを追加（7日 / 30日 / 90日 / 全期間）
* デフォルト: 7日
* 選択した期間のデータのみを `buildDataSummary()` で使用
* 期間選択はグラフタブの期間選択と独立して管理（`currentAIPeriod`）
* `buildDataSummary(period)` は引数で期間を受け取り、レコードをフィルタリング
* プロンプトに選択期間を明示（例: 「分析対象期間: 直近7日間」）
* `startAIDiagnosis()` と `sendFollowUp()` で `currentAIPeriod` を使用
* UI: `#ai-period-controls` 内のボタン群（グラフタブと同じスタイル `.chart-controls` を流用）
...
### 8.6 設定データ保存（localStorage）
* `sbpr_openai_api_key`: APIキー
* `sbpr_ai_model`: 利用モデルID（デフォルト: `gpt-4o-mini`）
* `sbpr_ai_memo`: AI向け備考
* `sbpr_day_start_hour`: 日中の開始時刻（0〜23、デフォルト: 6）
* `sbpr_night_start_hour`: 夜間の開始時刻（0〜23、デフォルト: 18）
* `sbpr_last_export_at`: 最終エクスポート日時（ISO 8601）。エクスポート成功時に保存。
* `sbpr_export_reminder_days`: エクスポート通知の間隔（日数）。1〜365。未設定時は7。
* `sbpr_export_reminder_enabled`: エクスポート通知の有効フラグ。`'0'` または `'false'` で無効、それ以外で有効。通知の「この通知を表示しない」でオフにすると無効になる。

### 8.7 AI API 使用量・コスト見積もり
