...
### 3.10 PWA（Progressive Web App）対応
...
* **PDFレポート共有**: html2pdf.js を使用して血圧レポート（統計サマリー・グラフ画像・記録一覧テーブル）をPDFとして生成し、Web Share API で共有またはダウンロードする。

### 3.11 PDFレポート機能
設定タブに「レポート共有」セクションを設け、PDFレポートの生成・共有機能を提供する。
* **レポート内容**:
    * ヘッダー（アプリ名 + 生成日時）
    * 統計サマリー（期間内の平均最高/最低血圧、平均脈拍、記録数）
    * 血圧推移グラフ（Chart.jsのcanvasを画像として埋め込み）
    * 直近の記録一覧テーブル（日時、最高血圧、最低血圧、脈拍、分類）
* **期間選択**: 現在のグラフタブで選択中の期間（7日/30日/90日/全期間）に連動
* **共有方法**:
    * Web Share API 対応端末: OS共有シートを表示（LINE、メール等で共有可能）
    * 非対応端末: PDFファイルを直接ダウンロード
* **ファイル名**: `sbpr_report_YYYYMMDD_HHMMSS.pdf`
