...
| E2E-002 | 血圧記録 | フォームに入力して保存 | ボタン直下に「記録を保存しました」が表示され、ボタンが一時「保存しました」に変わり、一覧に記録が追加される |
...
| E2E-028 | ヘッダータップでページ先頭へ戻る | ページ下部でヘッダーをクリック | ページが最上部にスクロールする（↑ボタンと同じ動作） |
| E2E-029 | AIチャット領域も先頭に戻る | AIチャットスクロール後に↑ボタン/ヘッダーをクリック | AIチャット領域のスクロールも先頭に戻る |
...
| E2E-015b | エクスポート後に最終エクスポート日時が保存される | 設定タブでエクスポートボタンをクリック | localStorageのsbpr_last_export_atにISO日時が保存される |
| E2E-015c | 服薬しなかった日を記録すると直近に「服薬なし」で表示される | 記録タブで日付を選び「服薬なしで保存」をクリック | ボタン直下に成功メッセージが出て、ボタンが一時「保存しました」に変わり、直近の記録に「服薬なし」が表示される |
...
| テストID | テスト内容 | 操作 | 期待結果 |
|----------|-----------|------|----------|
| E2E-030 | ツールチップの再タップ閉じ | グラフのデータポイントをタップしてツールチップ表示後、同じポイントを再タップ | ツールチップが閉じる |
| E2E-031 | 日中/夜間モードでの線種トグル表示 | 日中・夜間モードに切替 | 線種切替トグルが表示される |
| E2E-032 | 連続モードでの線種トグル非表示 | 連続モードに切替 | 線種切替トグルが非表示である |
| E2E-033 | 線種トグルでグラフ再描画 | 日中・夜間モードで線種トグルをクリック | グラフが再描画される（エラーなし） |

### 2.8 AI診断期間選択テスト
| テストID | テスト内容 | 操作 | 期待結果 |
|----------|-----------|------|----------|
| E2E-034 | AI診断タブに期間選択ボタンが表示される | AI診断タブを開く | 7日/30日/90日/全期間のボタンが表示される |
| E2E-035 | AI診断の期間選択ボタン切替 | 30日ボタンをクリック | 30日ボタンがactiveになり、7日ボタンのactiveが外れる |
