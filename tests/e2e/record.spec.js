...
test('E2E-050: メモクリアボタンでメモが消去される', async ({ page }) => {
    await page.goto('/', { waitUntil: 'networkidle' });
    await waitForAppReady(page);

    // メモに文字を入力
    await page.fill('#input-memo', 'テストメモ');
    const beforeClear = await page.evaluate(() => document.getElementById('input-memo').value);
    expect(beforeClear).toBe('テストメモ');

    // クリアボタンをクリック
    await page.click('#clear-memo-btn');

    // メモが空になる
    const afterClear = await page.evaluate(() => document.getElementById('input-memo').value);
